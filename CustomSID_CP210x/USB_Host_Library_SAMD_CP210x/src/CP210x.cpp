...
CP210x::CP210x(USBHost *p, CP210xAsyncOper *pasync) :
pAsync(pasync),
pUsb(p),
bAddress(0),
bNumEP(1)
...
uint32_t CP210x::Init(uint32_t parent, uint32_t port, uint32_t lowspeed) {
...
void CP210x::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
...
uint32_t CP210x::RcvData(uint16_t *bytes_rcvd, uint8_t *dataptr) {
        return pUsb->inTransfer(bAddress, epInfo[epDataInIndex].epAddr, bytes_rcvd, dataptr);
}

uint32_t CP210x::SndData(uint16_t nbytes, uint8_t *dataptr) {
